...
raw.data <- read.csv('./cov362_all_C_to_T40.csv', header = TRUE, row.names = 1)
...
if(T){
  ...
  samples_objects <- lapply(seq_along(samples_objects), function(i) {
    samples_objects[[i]] <- subset(x = samples_objects[[i]],
                                   subset = nFeature_RNA > 450 & percent.mt < 15 & nCount_RNA < 8500 & nFeature_RNA < 4000 & percent.ribo < 40 & percent.ribo > 5)
  })
...
if(T){
  ...
  if(F){
    ...
    all_cluster_colors <- list(
      c("0" = "#aa3474", "1" = "#ee8c7d"),   # C
      c("0" = "#aa3474", "1" = "#ee8c7d"),  # T5
      c("0" = "#aa3474", "1" = "#ee8c7d"),   # T10
      c("0" = "#aa3474", "1" = "#ee8c7d", "2" = "#1c8041"),   # T20
