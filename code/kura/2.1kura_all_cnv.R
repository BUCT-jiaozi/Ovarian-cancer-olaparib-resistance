...
library(Seurat)
data.merge <- readRDS(file = "./1.5data.merge.pro.rds")


if(F){
  index <- which(data.merge@meta.data$orig.ident %in% c("C")) # c("CD8+ T cell", "NK/NKT cell")
...
#possible.cancer.cell <- rownames(data.merge@meta.data)[which(data.merge@meta.data$seurat_clusters %in% c("4"))] #
# possible.cancer.cell <- rownames(data.merge@meta.data)[which(data.merge@meta.data$cellType_low %in% c("R1", "R2", "R3", "R4", "R5"))] #

if(F){
  possible.cancer.cell <- rownames(data.merge@meta.data)[data.merge@meta.data$cellType_low %in% c("R1", "R2", "R3", "R4", "R5") &
                                                           data.merge@meta.data$orig.ident %in% c("T1", "T2.5", "T5", "T10", "T20", "T40", "T80", "T160", "T320")] #
...
if(T){
  ...
  # get gene order
  gene_ordering = read.table("../../code/sup_files/hg38_gencode_v27.txt")
  # read matrices
  ...
  prepare_data_plot <- function(infercnv_obj_list_sample_ind, cell_ann_list_ind,
    ...
    infercnv_obj_list_sample = lapply(list(infercnv_obj_list_sample_ind),
                                      sample_cells, fraction = 0.05)  
    # infercnv_obj_list_sample = lapply(list(infercnv_obj_list[[1]]),
    #                                   sample_cells, fraction = 0.3) # 示例运行
  ...
  cluster_colors = list(c("R1" = "#7EB875", "R2" = "#57A2AC", "R3" = "#D1B9C5", "R4" = "#947D99", "R5" = "#6D4D6E"))
  
  
  sample_colors = list(c("T1" = "#B997C7", "T2.5" = "#824D99", "T5" = "#4E78C4", "T10" = "#57A2AC", "T20" = "#7EB875", 
                         "T40" = "#D0B541", "T80" = "#E67F33", "T160" = "#DD4E70", "T320" = "#CE2220"))
  ...
  pdf("./all_cnv_heatmaps.pdf", width = 10, height = 4.5)
  draw(
    list_heatmaps[[1]],
