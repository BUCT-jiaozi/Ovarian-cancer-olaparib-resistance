...
RNA <- readRDS("../1.5data.merge.pro.rds")
...
gene_id <- row.names(Neu_DEG_genes)[order(Neu_DEG_genes$qval)][1:1010] # 3000 1000 1200 2300
...
ClusterName_color_panel <- c("C" = "#1c8041", "R1" = "#fddf8b", "R2" = "#E55709",
                             "R3" = "#4E78C4", "R4"="#c77cff", "R5"= "#DD4E70")
# ClusterName_color_panel <- c("C" = "#1c8041", "R1" = "#fddf8b", "R2" = "#2e5fa1",
#                              "R3" = "#E55709", "R4"="#c77cff", "R5"= "#c94773")
library(cowplot)
...
# 展示基因在拟时序过程的表达变化
plot_genes <- c("KRT8", "CDKN2A","CD44") # "SOX17","WT1","ID1","SMAD3","RAD21", "RAD23B", "DDIT4"
cds_subset <- monocle_cds[plot_genes,] 


source("../../code/Functions/Monocle_plot_gene.R")
...
ggplot(plotdf, aes(x=Pseudotime,y=Neu_cluster_annotation,fill=Neu_cluster_annotation))+
  ...
  scale_fill_manual(
    name = "Cluster",
    values = c("#8DA3A6","#B997C7","#57A2AC", "#7EB875", "#4E78C4", "#DD4E70")
    )
ggsave("plot_out/4.4monocle_cluster_density.pdf",width = 6,height = 2.5)
...
BEAM_res <- BEAM(monocle_cds, branch_point = 1, cores = 6, progenitor_method = "duplicate")
