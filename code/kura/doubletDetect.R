...
doubletDetect <- function(Seurat.object, PCs, doublet.rate = 0.076, annotation = "seurat_clusters", pN_value = 0.25, GT = FALSE, sct = FALSE){
  ...
  T1.sample <- paramSweep(Seurat.object, PCs = PCs, sct = sct)
  ...
  pdf(file = paste0("./1.QualityControl/", Seurat.object@project.name, "_doublet.pdf"), width = 8, height = 6)# 加的,输出pdf
  ...
  if(is.null(annotation)){
    Seurat.object <- doubletFinder(Seurat.object, PCs = PCs, pN = pN_value, pK = pK_value, nExp = nExp_poi, reuse.pANN = FALSE, sct = sct)
    ...
    Seurat.object <- doubletFinder(Seurat.object, PCs = PCs, pN = pN_value, pK = pK_value, nExp = nExp_poi, reuse.pANN = FALSE, sct = sct)
    Seurat.object <- doubletFinder(Seurat.object, PCs = PCs, pN = pN_value, pK = pK_value, nExp = nExp_poi.adj, reuse.pANN = pANN_value, sct = sct)
