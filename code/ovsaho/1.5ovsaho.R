...
samples_objects <- lapply(seq_along(samples_raw_data), function(i) {
  ...
  pbmc.hashtag@meta.data$orig.ident <- recode(pbmc.hashtag@meta.data$HTO_maxID,
                                              "HTO-1-GTCAACTCTTTAGCG" = "C",
                                              "HTO-2-TGATGGCCTATTGGG" = "T5",
                                              "HTO-3-TTCCGCCTCTCTTTG" = "T10",
                                              "HTO-4-AGTAAGTTCAGCGTA" = "T20",
                                              "HTO-5-AAGTATCGTTTCGCA" = "T40")
...
names(samples_objects) <- c("SRR26822989", "SRR26822990", "SRR26822991", "SRR26822992")
...
names(samples_singlet) <- c("SRR26822989", "SRR26822990", "SRR26822991", "SRR26822992")
...
# # save table for GEO
# geo_counts = as.matrix(data.merge@assays$RNA@counts)
# write.table(geo_counts, file = "ovsaho_all_C_to_T40.csv", sep = ",", col.names = T, row.names = T,
#             quote = F)
...
### 质控，初步标准化分群
# 质控
data.merge <- subset(x = data.merge,subset = nFeature_RNA > 300 & percent.mt < 18 & nCount_RNA < 8500 & nFeature_RNA < 3500 & percent.ribo < 48 & percent.ribo > 8)
dim(data.merge)
...
data.merge <- FindClusters(object = data.merge, resolution = 0.6, verbose = FALSE)
clustree(data.merge)
...
DimPlot(object = data.merge, reduction = 'umap',label = TRUE, group.by = "RNA_snn_res.0.65")
DimPlot(object = data.merge, reduction = 'umap',label = TRUE, group.by = "orig.ident")


### 修改cluster编号顺序
if(F){
  Idents(data.merge) <- data.merge$RNA_snn_res.0.65
...
ratio.plot(seurat.object = data.merge, id.vars1 = "cellType_low", id.vars2 = "orig.ident", angle = 0, 
           color.len = c("#8DA3A6", "#7EB875", "#57A2AC", "#D1B9C5", "#947D99", "#6D4D6E"))
dev.off()
...
if(F){
  ...
  CellDimPlot(data.merge, group.by = "Clusters", reduction = "UMAP", theme_use = "theme_blank", #theme_blank theme_void theme_linedraw theme_light
              title = "OVSAHO",
              palcolor = c("#8DA3A6","#57A2AC", "#4E78C4", "#B997C7", "#824D99", "#7EB875"),
              #theme_use = ggplot2::theme_classic, 
              #cells.highlight = colnames(pancreas_sub)[pancreas_sub$SubCellType == "Epsilon"],
              #label = TRUE, label_insitu = TRUE, label_repel = TRUE, label_segment_color = "red",
              legend.position  = "right", legend.direction = "vertical",
    ...
    scale_color_manual(
      name = "Clusters:",
      values = c("#8DA3A6","#57A2AC", "#4E78C4", "#B997C7", "#824D99", "#7EB875"),
      guide = guide_legend(
        override.aes = list(size = 4)  # 👈 图例圆点大小
      ))
  dev.off()
  ...
  FeatureStatPlot(
    ...
    #bg.by = "celltype",
    stat.by = c("SOX17","SLC1A3"),  #"WT1", "IFI6", "IFI27", "CD44", "KLF4", "SGK1", "CITED2",
    #"PARP1", "RAD23A", "CDK1", "ID1", "RPL17", "DAB2",
    add_box = TRUE,
  )
}
...
data.merge_avg_bulk = data.merge_avg_bulk[, c(1,4,2,3,5)] # 调整顺序
...
genes_to_show = c("SOX17", "PARP1", "WT1", "S100A16", "CDK14",
                  "CD47", "KRT8", "BCAM", "CDKN2C", 
                  "CCNB1", "CCNB2", "CCND1", "PARP14", "CDC20",  
                  "PAX8", "ID1", "CD24", "S100A11", "FN1", "CDKN2A", "ID2", "RAD23A", 
                  "LBH", "DDIT4", "DDIT3",
                  "RAD21", "CD44", "SMAD3", "MKI67", "TOP2A")
...
avg_expression$id <- factor(avg_expression$id, levels = c("c0", "c1", "c2", "c3", "c4", "c5"))

# 自定义颜色映射
id_colors <- c("#8DA3A6","#57A2AC", "#4E78C4", "#B997C7", "#824D99", "#7EB875")

# 根据你的要求对基因进行分组颜色映射
gene_colors <- c(rep("#8DA3A6", 5), rep("#57A2AC", 4), rep("#4E78C4", 5),
                 rep("#B997C7", 8), rep("#824D99", 3), rep("#7EB875", 5))
